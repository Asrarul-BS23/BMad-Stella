...
shared-rules:
  ...
  architecture-pages: 'Architecture pages = direct child pages of `architectureFolderUrl` EXCLUDING any child whose title is or starts with "Domain-Knowledge" (case-insensitive). That page belongs to the domain-expert agent — never count, fetch, or save it in STEP 5-7.'
activation-instructions:
  ...
  - STEP 5: Cache check. If `bmad-docs/architecture/.metadata.json` exists and parses as valid JSON, fetch metadata-only (pageId + version.number) for each architecture page (per `architecture-pages` rule) of `architectureFolderUrl` via Atlassian MCP. On MCP failure → apply `mcp-failure` rule. Cache is valid when architecture page count matches manifest `pages` length AND every manifest entry's `version` equals the live Confluence `version.number` AND every `bmad-docs/architecture/{localFile}` exists on disk. If cache is valid, skip STEP 6 and STEP 7 and proceed to STEP 8. Otherwise fall through to STEP 6
  - STEP 6: Delete existing `bmad-docs/architecture/` folder if present using "Bash(rm -rf bmad-docs/architecture)", create fresh `bmad-docs/architecture/` directory, then fetch every architecture page (per `architecture-pages` rule) under `architectureFolderUrl` using Atlassian MCP. On MCP failure → apply `mcp-failure` rule, then retry STEP 6. Do NOT proceed to STEP 7 until documentation fetch succeeds
  - 'STEP 7: Before any greeting, save each fetched page as a separate file inside `bmad-docs/architecture/`. Name files from the page title: lowercase, hyphenated, project suffix removed. Pages whose title starts with Coding-Standards, Tech-Stack, Project-Structure, or Git-Workflow MUST be saved as exactly coding-standards, tech-stack, project-structure, git-workflow (these paths are hardcoded in core-config LoadAlwaysFiles). Verify number of files created matches number of architecture pages (per `architecture-pages` rule). Then write `bmad-docs/architecture/.metadata.json` with shape `{"pages": [{"pageId", "title", "version", "localFile"}, ...]}` — one entry per saved page, `version` is the Confluence `version.number`, `localFile` is the filename written (no directory prefix). Do NOT proceed to STEP 8 until all architecture docs AND the manifest are successfully saved'
