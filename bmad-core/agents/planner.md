...
commands:
  ...
  - retrieve-ticket-information {ticket-number-or-url}:
      ...
      - attachment-auto-fetch:
          ...
          - 'If the error is credentials/auth (exit 10/20, "Missing credentials" / "Authentication failed"), append: "Check JIRA_BASE_URL / JIRA_EMAIL / JIRA_API_TOKEN in .env; regenerate token: https://id.atlassian.com/manage-profile/security/api-tokens". If the ticket was not found (exit 30), ask the user to verify the ticket key instead of pasting.'
