...
activation-instructions:
  ...
  - 'STEP 7: Before any greeting, save each fetched page as a separate file inside `bmad-docs/architecture/`. Name files from the page title: lowercase, hyphenated, project suffix removed. Pages whose title starts with Coding-Standards, Tech-Stack, Project-Structure, or Git-Workflow MUST be saved as exactly coding-standards, tech-stack, project-structure, git-workflow (these paths are hardcoded in core-config LoadAlwaysFiles). Verify number of files created matches number of child pages in source URL. Then write `bmad-docs/architecture/.metadata.json` with shape `{"pages": [{"pageId", "title", "version", "localFile"}, ...]}` — one entry per saved page, `version` is the Confluence `version.number`, `localFile` is the filename written (no directory prefix). Do NOT proceed to STEP 8 until all architecture docs AND the manifest are successfully saved'
