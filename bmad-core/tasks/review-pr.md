...
- **Pull request** — fetch via GitHub MCP from `pr_url`: use `pull_request_read` (method `get` for title/description/SHAs, then `get_diff` / `get_files` for the changed files + hunks). The diff is the implementation under review. GitHub MCP is the ONLY source — never run local `git` and never assume a local checkout; the repo on disk may not contain the PR's commits. If a GitHub MCP call fails (e.g., a 403 on `get_diff`), HALT and report it — most often the token is missing `Contents: Read` (a Pull-requests-only token can read metadata but not the diff). Do NOT fall back to local `git`.
...
Change set = the PR diff fetched during Context Gain — the files and hunks GitHub reports as changed. Review each added or modified hunk in context; when a hunk needs surrounding code to judge it, fetch that file at the PR's head ref via GitHub MCP `get_file_contents` — never run local `git` (e.g. `git diff`, `git show`) and never read from a local working copy. For deletions, judge the removal from the diff and flag any regression or lost validation/tests it causes.
